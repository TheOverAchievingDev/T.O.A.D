...
/**
 * Run the eas CLI with given args and return { stdout, stderr, exitCode }.
 * Supports log streaming via onLog callback for background jobs.
 */
export async function runEasCli({
  args,
  cwd = process.cwd(),
  timeoutMs = 600_000, // EAS builds can be slow, 10 min default
  stdin = null,
  spawnImpl,
  onLog, // Callback for streaming logs: (chunk: string) => void
} = {}) {
  if (!Array.isArray(args)) {
    throw new TypeError('runEasCli: args must be an array');
  }
  const spawnFn = spawnImpl || defaultSpawn;
  const cliPath = resolveCommandPath('eas');
  ...
  return await new Promise((resolve, reject) => {
    ...
    try {
      ...
      reject(new Error(`eas spawn failed: ${err && err.message ? err.message : err}`));
      return;
    }
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`eas timed out after ${timeoutMs}ms`));
    }, timeoutMs);
    ...
    proc.on('error', (err) => {
      if (timedOut) return;
      clearTimeout(timer);
      reject(new Error(`eas spawn error: ${err.message}`));
    });
  });
}
