...
6. Smaller follow-ups: an optional `--bare` smoke path for users on API-key auth; a companion `agent_stop` MCP tool + dashboard stop button (now that `agent_launch` is in place).
