...
6. Next legacy-parity slices (in priority order): team lifecycle (`team_create`/`team_launch`/`team_stop`/`team_list`/`team_delete` to manage teams as a unit, currently only individual agents), code review with diffs (legacy `review.ts` stores per-task diff content for file-level accept/reject; TOAD's `review_request`/`review_decide` work on task IDs only), notifications (typed alert categories on top of the raw event bus), and `team_process_send` (target a specific runtime's stdin). After those: an optional `--bare` smoke variant for users on API-key auth, and Codex provider integration.
