...
5. Restart housekeeping telemetry now flows on the SSE bus: `side_effects_dropped_on_restart` and `side_effects_pruned` events are emitted from `LocalToadRuntime.start()` whenever the corresponding pass did non-zero work. A dashboard system panel that consumes them is the natural UI follow-up.
6. Smaller follow-ups: a periodic `VACUUM` to reclaim disk pages from high-churn installs, and persisting `TOAD_API_TOKEN` to a config file rather than shell env for long-running daemons.
