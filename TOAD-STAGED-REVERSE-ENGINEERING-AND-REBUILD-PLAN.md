...
6. Team lifecycle is complete end-to-end: persistent `team_create` / `team_list` / `team_delete` plus orchestration `team_launch` / `team_stop`. Runtime IDs are derived as `runtime-<teamId>-<agentId>` so re-launches are idempotent and `team_stop` can find what `team_launch` started without separate bookkeeping. Per-member failures are recorded but do not abort the rest of the team. `runtime_send_input` covers the legacy `TEAM_PROCESS_SEND` path: writes text directly to a runtime's stdin via its adapter, bypassing the broker. Code review with diffs is now in: `review_request` accepts `summary`/`diff`/`files`, `review_decide` accepts per-file `feedback`, and a new read-only `review_list` returns tasks with active reviews including the diff content. The `task.review` projection cohesively groups everything review-related under one sub-object. Next legacy-parity slices: notifications (typed alert categories on the event bus), git provisioning hooks (auto-generate diffs from worktrees), file/hunk-level review reject, optional `--bare` smoke variant, and Codex provider integration.
