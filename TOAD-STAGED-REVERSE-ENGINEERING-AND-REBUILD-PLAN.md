...
6. Team lifecycle is complete end-to-end: persistent `team_create` / `team_list` / `team_delete` plus orchestration `team_launch` / `team_stop`. Runtime IDs are derived as `runtime-<teamId>-<agentId>` so re-launches are idempotent and `team_stop` can find what `team_launch` started without separate bookkeeping. Per-member failures are recorded but do not abort the rest of the team. `runtime_send_input` covers the legacy `TEAM_PROCESS_SEND` path. Code review with diffs is in: `review_request`/`review_decide`/`review_list` carry diff content, per-file feedback, and surface open reviews via a read tool. The deterministic state machine (§3 of the v2 hardening checklist) is now enforced: `task_update` validates every transition against `ALLOWED_TRANSITIONS`, throws on illegal moves, and records `from`/`reason` per checklist requirement. Role authority (§5 + §26) is also enforced: `actor.role` is optional input on every command, `LocalToolFacade.execute` calls `assertRoleCanCallTool` against per-role allowlists (`lead`/`human` are wildcard; `architect`/`developer`/`reviewer`/`tester` get explicit lists), and `#reviewDecide` blocks self-review (same agent that requested cannot decide). Missing role defaults to `human` for backward compat with prior test fixtures. Test artifacts + CI gates (§6/§18) are now wired: `TeamConfig.validation` carries the install/lint/typecheck/test/build/security commands; the new `validation_run` MCP tool spawns the configured command (DI'd `spawnValidation` for tests), captures exit code/stdout/stderr/duration, records a `task.validation_run` event, and `#taskUpdate` blocks `testing → merge_ready` unless `task.latestValidation.test?.verdict === 'passed'`. Plan-before-code (§2) is also enforced: three new task event types (`task.plan_proposed`/`approved`/`rejected`), three new MCP tools (`task_plan_propose`/`task_plan_approve`/`task_plan_reject`), `task.plan` projection, and a `ready → planned` gate that requires `task.plan?.state === 'approved'`. Self-approval prevention applies to plans like reviews. Slice order anchored to the v2 checklist — see `toad-local/docs/CHECKLIST_GAP_MATRIX.md` for the live gap matrix. Next: diagnostics (§25), worktree enforcement (§8), per-transition role guards, then smaller follow-ups.
