...
6. Team-lifecycle CRUD half is complete: `team_create` / `team_list` / `team_delete` MCP tools backed by a persistent `SqliteTeamConfigRegistry` (joins the other five SQLite stores in `<projectCwd>/.toad/toad.db`). The orchestration half (`team_launch` / `team_stop`) is the next slice. After that: code review with diffs (legacy `review.ts` stores per-task diff content; TOAD's `review_request`/`review_decide` work on task IDs only), notifications (typed alert categories on the event bus), `team_process_send` (target a specific runtime's stdin), an optional `--bare` smoke variant for API-key users, and Codex provider integration.
