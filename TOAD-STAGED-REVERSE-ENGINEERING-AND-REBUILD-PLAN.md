...
6. Team lifecycle is complete end-to-end: persistent `team_create` / `team_list` / `team_delete` plus orchestration `team_launch` / `team_stop`. Runtime IDs are derived as `runtime-<teamId>-<agentId>` so re-launches are idempotent and `team_stop` can find what `team_launch` started without separate bookkeeping. Per-member failures are recorded but do not abort the rest of the team. `runtime_send_input` covers the legacy `TEAM_PROCESS_SEND` path. Code review with diffs is in: `review_request`/`review_decide`/`review_list` carry diff content, per-file feedback, and surface open reviews via a read tool. The deterministic state machine (§3 of the v2 hardening checklist) is now enforced: `task_update` validates every transition against `ALLOWED_TRANSITIONS`, throws on illegal moves, and records `from`/`reason` per checklist requirement. Slice order is now anchored to the v2 checklist's own priority — see `toad-local/docs/CHECKLIST_GAP_MATRIX.md` for the gap matrix. Next: role authority (§5/§26), test artifacts + CI gates (§6/§18), plan-before-code (§2), diagnostics (§25), worktree enforcement (§8), then smaller follow-ups.
