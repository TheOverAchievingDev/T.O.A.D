...
6. Smaller follow-ups: persisting `TOAD_API_TOKEN` to a config file rather than shell env for long-running daemons; an optional `--bare` smoke path for users on API-key auth.
