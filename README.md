...
- **Drift monitor slice 2.** LLM-semantic check tier, taskBoard fan-out for off-cycle drift runs, lift `useDrift` to App-level (Workspace + TasksScreen currently double-poll), enrich `buildSnapshot` with `fileContents` so `checkProviderLogicLeakage` fires on real diffs, register `drift_run` in MCP tool definitions for non-UI callers.
