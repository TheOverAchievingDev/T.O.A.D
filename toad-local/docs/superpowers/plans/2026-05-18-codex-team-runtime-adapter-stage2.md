...
### Whole-Impl Review — Stage 2 code-quality outcome (RATIFIED 2026-05-18, user-approved "fix all now")

> **Process:** Two independent Opus reviews (a dispatched senior-architect reviewer with clean crafted context + the controller's independent line-by-line verification of the epicenter) **converged**. Verdict: **NOT ready to merge** — two empirically-reproduced spec-§5/§8 lost/duplicate-message defects in the *shared, all-provider* delivery path, both untested. Sound: the pure `_chain` FIFO serializer (race-free; chain-poisoning-safe), single-source terminal `turn_failed` on session-loss, the byte-unchanged Claude invariant (commit-level **and** behavioural), and additive/idempotent migration. The recurring defect family all stems from `listMessagesNeedingDelivery` + the idempotency gate inferring "needs delivery" from `(status, response_state)` snapshots with **no explicit in-flight/claimed state**. Approved fix: make the delivery-attempt lifecycle an explicit state machine — `queued_for_recipient` → (worker claims) `delivering` → success `accepted_by_runtime` (terminal) | failure `queued_for_recipient` (re-drive, **bounded** by a retry cap) ; boot resets stale `delivering` → `queued_for_recipient` (a `delivering` attempt means the process died mid-turn). Every change session_turn-scoped or behind the existing gates — **Claude `runtime_stdin`/`runtime_bridge` provably byte-unchanged**; the full deliveryWorker + broker + codex + localToadRuntime regression must stay green (load-bearing, all-provider).

**Findings → fix tasks (TDD red→green, one commit each, commit-hygiene gate per task):**

- [ ] **W1 — Critical 1: parked re-entry storm.** The bare `setInterval(tick,500)` sweep re-enters `deliverMessage` every 500 ms while a session attempt is still `queued_for_recipient` (the multi-minute turn hasn't committed) → same message pushed into `_pendingTexts` N×. **Fix:** DeliveryWorker claims the attempt `delivering` (new broker `markDeliveryInFlight`) *before* `await adapter.sendTurn` (session mode only); the existing idempotency gate already early-returns for any committed non-`queued_for_recipient`, and `listMessagesNeedingDelivery` already only matches `queued_for_recipient`, so a `delivering` attempt is doubly excluded from re-entry. Files: `src/delivery/deliveryWorker.js`, `src/broker/sqliteBroker.js`, `src/broker/inMemoryBroker.js`, new `test/codex/deliveryWorker.inFlightReentry.test.js` (RED: parked msg + slow `sendTurn` + concurrent 2nd `deliverMessage` ⇒ `sendTurn` called exactly once). Commit: `fix(delivery): claim session attempt in-flight before sendTurn — no parked-message re-entry storm (SP1a Stage 2, ratified whole-impl C1)`.
- [ ] **W2 — Critical 2: failed wake permanently dropped.** A session wake that fails while live → `failDeliveryAttempt(failed_retryable/delivery_failed)` is never re-surfaced by the sweep (only `queued_for_recipient`/offline) → lost until restart (violates §5/§8). **Fix:** on session-mode `sendTurn` failure, re-commit the attempt to `queued_for_recipient` (sweep re-drives it), bounded by a `retryCount` carried in `receipt_json`; past the cap → `failDeliveryAttempt(retryable:false)` + honest warn (terminal, no infinite storm). Files: `src/delivery/deliveryWorker.js`, new `test/codex/deliveryWorker.failedWakeRedrive.test.js` (RED: failed wake ⇒ still in `listMessagesNeedingDelivery`; after CAP ⇒ terminal, not surfaced). Commit: `fix(delivery): failed session wake is bounded-redrivable, never silently dropped (SP1a Stage 2, ratified whole-impl C2)`.
- [ ] **W3 — crash recovery for W1's claimed state.** A `delivering` attempt whose process died mid-turn is undelivered again on restart; the running sweep won't re-surface it (not `queued_for_recipient`). **Fix:** boot reconciliation resets stale session `delivering` → `queued_for_recipient` (new broker `resetStaleSessionInFlight`) before computing undelivered. Files: `src/app/LocalToadRuntime.js`, `src/broker/sqliteBroker.js`, `src/broker/inMemoryBroker.js`, extend `test/codex/reconcileSessionInboxes.test.js` (RED: `delivering` attempt + boot ⇒ message re-driven). Commit: `fix(delivery): boot resets stale in-flight session deliveries for re-drive (SP1a Stage 2, ratified whole-impl C1-crash)`.
- [ ] **W4 — Important 4: dead `hasCommittedRuntimeDelivery`.** The method does not exist on either broker → `isCommitted` is unconditionally `false` → boot re-drives the *entire* inbox (amplifies W1). **Fix:** implement real `hasCommittedRuntimeDelivery(messageId)` on both brokers (true iff a committed runtime_stdin/runtime_bridge **or** a session_turn `accepted_by_runtime`/`coalesced` attempt exists); drop the dead `?:` at the call site. Files: `src/broker/sqliteBroker.js`, `src/broker/inMemoryBroker.js`, `src/app/LocalToadRuntime.js`, new `test/codex/hasCommittedRuntimeDelivery.test.js`. Commit: `fix(broker): real hasCommittedRuntimeDelivery — boot reconcile no longer re-drives delivered inbox (SP1a Stage 2, ratified whole-impl I4)`.
- [ ] **W5 — Important 3: sync spawn-throw loses the coalesced batch.** `#runTurn` clears `_pendingTexts` before the spawn is confirmed; a synchronous `spawnImpl` throw discards the batch while later chained slots resolve a false `coalesced` success. **Fix:** clear `_pendingTexts` only once the spawn is confirmed; restore the batch on a pre-spawn throw so the next chained slot re-drains it (no silent loss, no false success). Files: `src/runtime/CodexExecAdapter.js`, new `test/codex/codexExecAdapter.spawnThrow.test.js`. Commit: `fix(codex): pre-spawn throw must not discard the coalesced batch as false success (SP1a Stage 2, ratified whole-impl I3)`.
- [ ] **W6 — Minors (one commit, green-keeping):** (6) no-op guard in the session store when `cliSessionId` is unchanged (no per-turn `UPDATE`/`updated_at` churn); (7) tighter `UNKNOWN_SESSION_RE` anchored to Codex's real resume-failure text; (8) per-runtime try/catch around the monitor's `getAdapter` probe so one throw can't abort the recovery sweep. Files: `src/runtime/codex/runtimeRegistrySessionStore.js`, `src/runtime/CodexExecAdapter.js`, `src/diagnostics/stuckRuntimeMonitor.js` + targeted tests. Commit: `refactor(codex): whole-impl review minors — session-store no-op guard, tighter unknown-session regex, monitor probe guard (SP1a Stage 2, ratified whole-impl M6-8)`.

> After W1–W6: controller re-runs the **full root-suite gate** (`bash -c "$(cat scripts/test-suites.txt)"` from `toad-local/`), confirms `EXIT=0` + summed `fail 0`, reconciles the pass count vs the prior baseline + the new W-task cases, re-confirms the out-of-scope diff EMPTY (Claude/foundry/normalizer/UI byte-unchanged). Then `superpowers:finishing-a-development-branch`.
