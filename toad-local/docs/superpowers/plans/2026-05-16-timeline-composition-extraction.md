...
[
  ...
  { "name": "drift-only: surfaces drift rows in the golden (>=3 emit, <3 skip, cap-2)", "input": { "now": 1747353600000, "agents": [], "agentStreams": {}, "taskTransitions": [], "driftHistory": [ { "runId": "dz1", "teamScore": 40, "createdAt": "2026-05-16T09:00:00.000Z" }, { "runId": "dz2", "teamScore": 41, "createdAt": "2026-05-16T09:01:00.000Z" }, { "runId": "dz3", "teamScore": 55, "createdAt": "2026-05-16T09:02:00.000Z" }, { "runId": "dz4", "teamScore": 70, "createdAt": "2026-05-16T09:03:00.000Z" } ] } }
]
```

> **CONTROLLER-RATIFIED (T1, fixture-coverage §8d).** The pristine `projectTimeline` assigns drift rows a synthetic `_ts = (driftHistory.length)*1000 - i` (tiny vs the real-epoch-ms stream/lifecycle `_ts`), so drift rows sort last and are capped out of the final `slice(0,limit)` whenever `stream+lifecycle ≥ limit` — the first three fixture cases therefore yield **zero drift rows in the golden**, leaving the strict agreement gate (Task 7) with a blind spot on the drift composition + `driftEl` path. This 4th **drift-only** case (no streams/lifecycle, 4 `driftHistory` entries: `Δ1` skip, `Δ14` emit, `Δ15` emit → cap-2) makes `merged` consist solely of `driftWithTs`, so the drift rows survive the cap and the golden+agreement byte-cover `driftEl` end-to-end. Faithful to pristine behavior (the quirk is preserved, just exercised); the golden is still a one-time capture from pristine `projectTimelineGolden`, only over a more complete fixture.
