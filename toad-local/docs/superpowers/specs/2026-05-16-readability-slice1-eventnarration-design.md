...
- **`agentStream.eventToStreamEntry()`** — replace its per-tool wording (`summarizeToolInput`) and its event→`kind` classification with `narrate()` **for the event types this consumer actually narrates** (`assistant_text`, `tool_use`, `turn_completed` when `raw.result`, `approval_request`): compute `const n = narrate(event)`, set `StreamEntry.body = n.line`, and derive `StreamEntry.kind` from `n.kind` via a pinned **`NarrationKind` → `StreamEntry.kind` adapter** (`tool→'tool'`, `text→'output'`, `system→'system'`). `summarizeToolInput` is **deleted** (its logic moves into `narrate()`). **`runtime_event` is consumer-owned in Slice 1 — its branch is left UNCHANGED** (its own `task_started`/`post_turn_summary` selection AND wording, including the legacy `'thought'` `StreamEntry.kind`, stay in `eventToStreamEntry`; `deriveAgentActivity` likewise keeps its own `runtime_event`/`task_started`→`'Working…'` handling). `narrate()` *does* contain a `runtime_event` branch purely for **totality / degraded-safety and post-C readiness**, but **no Slice-1 consumer routes `runtime_event` through `narrate()`**, so it is NOT a consolidation behavior change and NOT an agreement-test divergence (see §5 scope). The remaining `null`-drop decisions (`assistant_text` with no text, `runtime_event` lifecycle, `turn_completed` without `raw.result`) stay in `eventToStreamEntry` (selection, not wording).
