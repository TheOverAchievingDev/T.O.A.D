...
- **Codex consumes MCP servers in `exec`.** Configured via `config.toml` `[mcp_servers.*]`; **project-scoped `.codex/config.toml` (trusted projects only)** works without touching `~/.codex/config.toml`. An enabled server with `required = true` makes `codex exec` **exit non-zero** if it fails to initialize (no silent mute agent). `--json` explicitly emits `mcp_tool_call` items, confirming MCP tools are usable during `exec`. Known reliability footnotes exist (openai/codex#3441, #21654) → mitigated by `required = true` + a first-turn visibility probe + explicit project-trust setup.
- **Codex governance flags:** `--sandbox read-only|workspace-write|danger-full-access`, `--ask-for-approval untrusted|on-request|never`, `--skip-git-repo-check`, `-C/--cd <path>`, `--json`, `--ephemeral`, repeatable `-c/--config key=value`; `--full-auto` deprecated (use `--sandbox workspace-write`).
...
- **`sendTurn(input)`** (reuses `CodexFoundryAdapter` argv shapes):
  - *First turn* (no `cliSessionId`): `codex exec --json --skip-git-repo-check -C <cwd> --sandbox workspace-write --ask-for-approval never -`; stdin = `systemPrompt\n\nuserMessage` (`codex exec` has no append-system-prompt flag — prepend on turn 1; persistent conventions live in `AGENTS.md`).
  - *Resume*: `codex exec resume --json <cliSessionId> -`; stdin = `userMessage` only (prior convo+instructions on disk; resume rejects `-C`/`--skip-git-repo-check`).
...
- TOAD writes a per-agent project-local **`.codex/config.toml`** into the agent's worktree cwd with `[mcp_servers.toad]` pointing at the **same** TOAD MCP server Claude uses, `required = true` (a failed rail makes the turn fail **loudly**). Mirrors the existing per-agent `.claude/settings.local.json` writer — same pattern, Codex's format.
- **Trust setup:** project-scoped `.codex/config.toml` loads only for *trusted* projects. The materializer marks the agent's worktree trusted as a required, explicit launch step (not assumed).
...
- **Sandbox policy:** team-Codex launches with `--sandbox workspace-write --ask-for-approval never` — autonomous within the per-task worktree, cannot write outside it, no interactive pause. **Not** `danger-full-access`/`--yolo`. (Per-team override is a later knob — out of SP1a scope.)
...
- **MCP rail failed to load:** `required = true` ⇒ non-zero exit ⇒ `turn_failed("TOAD tools unavailable")` + first-turn visibility probe. Never a silent mute agent.
...
**§8d grounding pins — the implementer MUST re-verify these against the *installed* `codex` CLI + shipped code before writing each step (CLI drift is a real risk: `CodexFoundryAdapter` pinned 0.117.0 @ 2026-04-19; current ≥ 0.130):**

- The exact `codex exec` + `codex exec resume` argv, the `-` stdin sentinel behavior, and that `--sandbox workspace-write`/`--ask-for-approval never`/`--skip-git-repo-check`/`-C`/`--json`/`--ephemeral` are accepted by the installed version (re-derive from `CodexFoundryAdapter` + `codex --help`/docs; do not assume).
- The exact `--json` item vocabulary (`thread.started`, `item.completed` types incl. `agent_message`/`command_execution`/`file_change`/`mcp_tool_call`/`reasoning`/`plan_update`, `turn.completed`, error shapes) against a real run/the installed version.
- That `codex exec` honors a project-scoped `.codex/config.toml [mcp_servers.toad]` with `required = true` and the precise project-**trust** mechanism/command, by actual probe (the front-loaded proof) — not docs alone (openai/codex#3441/#21654 reliability footnotes).
