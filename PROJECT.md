...
|---|---|---|
| **Lives in** | Repository root (this repo) | Any folder the user picks |
...
The workspace is whatever directory the user picked when creating the project. The Symphony app's own code (this repo) is not the workspace. The user's home directory is not the workspace. The OS root is not the workspace.
...
- `projectCwd === installDir` (the exact bug class: sidecar booted with the Symphony install directory as the user workspace).
...
- **The Tauri shell** spawns the Node API server with the correct `TOAD_PROJECT_CWD` env var derived from the user's saved active project. External launcher scripts must not spawn competing API servers, because they can race Tauri's spawn for port 3001 and win with the wrong `projectCwd`.
...
|---|---|---|
| **Sidecar cwd** | The Node process's cwd. Set by Tauri's sidecar launch or manual `node` invocation. Stored as `this.projectCwd` in `localToolFacade.js`. | `C:\Users\Nova_\projects\meal-planner` (after `switch_project` fired) or `C:\Project-TOAD` (default when launched from the repo) |
